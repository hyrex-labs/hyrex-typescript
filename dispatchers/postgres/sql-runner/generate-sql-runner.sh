...
generate_method_signature() {
  ...
  # Convert to camelCase - first character to lowercase
  local first_char=$(echo "${name:0:1}" | tr '[:upper:]' '[:lower:]')
  local method_name="${first_char}${name:1}"
  
  if [ -z "$params" ]; then
    echo "  ${method_name}(): Promise<QueryResultRow[]>;"
  else
    ...
    for param in $params; do
      ...
      case "$param" in
        ...
        *Args|*Info|*Structure|stats|*Result) param_type="Record<string, unknown>" ;;
      esac
    ...
    echo "  ${method_name}(${param_list}): Promise<QueryResultRow[]>;"
  fi
}
...
# Add a reference to the custom types
cat >> "$OUTPUT_FILE" << EOF
