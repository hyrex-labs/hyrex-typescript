...
export const CreateWorkflowRunTable = `
    DO $$
BEGIN
...
export const SET_WORKFLOW_RUN_STATUS_BASED_ON_TASK_RUNS = `
    WITH latest_attempts AS (
        ...
        ORDER BY durable_id, attempt_number DESC),
         workflow_statuses AS (SELECT workflow_run_id,
                                      CASE
                                          ...
                                          WHEN EXISTS (SELECT 1
                                                       ...
                                                       WHERE latest_attempts.workflow_run_id = w.id
                                                         AND status = 'failed'
                                                         AND attempt_number >= max_retries) THEN 'failed'::workflow_run_status
                                          ...
                                          WHEN NOT EXISTS (SELECT 1
                                                           FROM latest_attempts
                                                           WHERE latest_attempts.workflow_run_id = w.id
                                                             AND status NOT IN ('success', 'skipped')) THEN 'success'::workflow_run_status

                                          -- Handle other states (lost, canceled)
                                          WHEN EXISTS (SELECT 1
                                                       FROM latest_attempts
                                                       WHERE latest_attempts.workflow_run_id = w.id
                                                         AND status IN ('lost', 'canceled')) THEN 'failed'::workflow_run_status
    ...
    WHERE w.id = ws.workflow_run_id
      AND w.status != ws.new_status
    RETURNING w.id, w.status;
`

export const ADVANCE_WORKFLOW_RUN = `
    WITH latest_attempts AS (
        -- Get the latest attempt for each durable_id
        SELECT DISTINCT ON (durable_id) durable_id,
                                        status
